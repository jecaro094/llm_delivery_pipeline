...
def test_deserialize_rejects_the_pre_signing_manifest_version() -> None:
    ...
    """
    with pytest.raises(manifest.ManifestError, match="manifest_version"):
        manifest.deserialize_manifest(_tampered_bytes(manifest_version="1.0"))
...
def test_serialized_manifest_bytes_match_a_committed_fixture() -> None:
    ...
    expected = (
        ...
        b'  "signature": {\n'
        b'    "algorithm": "Ed25519",\n'
        b'    "public_key_sha256": "' + b"c" * 64 + b'",\n'
        b'    "signature_path": "versions/1.0.0/manifest.json.sig"\n'
        b"  }\n"
        b"}"
    )
