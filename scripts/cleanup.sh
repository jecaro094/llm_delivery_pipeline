...
# the consumer Pod, the encryption-key and signing-key Secrets, and the
# signing public-key ConfigMap. Idempotent -- safe to run when nothing
...
echo "== Deleting workloads, secrets, and configmaps in ${NAMESPACE} =="
...
kubectl -n "${NAMESPACE}" delete secret model-signing-key --ignore-not-found
kubectl -n "${NAMESPACE}" delete configmap model-signing-public-key --ignore-not-found
